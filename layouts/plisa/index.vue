...
const { nowaWycena, dodajProdukt, selectProdukt, cenaM, updateProduct } =
    wycenyStore();

const moskitiery = noweProdukty.value.find(
    ({ produkt }) => produkt.toLowerCase() === selectedProdukt.value.produkt
);
...
<template>
    ...
    >
        <span
            class="absolute top-[-13px] text-slate-600 bg-white flex justify- font-semibold"
            >Rodzaj Plisy</span
        >
        ...
        <span
            ...
            >{{ currentProdukt.oznaczenie }}</span
        >
        <label
            v-for="item in moskitiery.rodzaje"
            :for="item.name"
            :class="[
